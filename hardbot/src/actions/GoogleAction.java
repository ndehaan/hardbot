...
public class GoogleAction extends Action {
	private final String HTTP_REFERER = "http://www.ubc.ca/";
	...
	public String[] help() {
		...
		return new String[] {
				"Returns the top 3 results of a Google search for a given query.",
	...
	public String[] perform(String request, String sender, MyBot bot) {
		...
		try {
			...
			String query = URLEncoder.encode(request, "UTF-8");
			...
			resultList.add(json.getJSONObject("responseData").getJSONObject(
			...
			for (int i = 0; i < 3; i++) {
				...
				resultList.add(result + j.getString("titleNoFormatting"));

				resultList.add("   " + unescape(j.getString("url")));
			}
		} catch (Exception e) {
